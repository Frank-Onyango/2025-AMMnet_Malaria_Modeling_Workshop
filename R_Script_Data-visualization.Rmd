...
```{r}
#setwd("D:/APHRC/Malaria Modelling Training/2025-AMMnet_Malaria_Modeling_Workshop-main")    # It tells R where to look for files and where to save files
...
p2<-ggplot(SMdata) +
 ...
 direction = 1) +
 labs(title = "Boxplot", caption = "Source: Frank, 2025") +
 coord_flip() +
 theme_classic() +
 theme(legend.position = "top", plot.title = element_text(size = 20L, face = "bold", hjust = 0.5), 
 plot.caption = element_text(face = "bold.italic", hjust = 0), axis.text.y = element_text(size = 20L), 
...
p4<-ggplot(SMdata) +
 aes(x = Positive) +
 geom_histogram(bins = 5L, fill = "#9ED79E") +
...
p5<-ggplot(SMdata) +
 aes(x = Total, y = Positive, colour = Region) +
 geom_point(size = 3L, shape = "asterisk") +
 scale_color_hue(direction = 1) +
 labs(title = "Scatter plot") +
 theme_minimal() +
 theme(legend.position = "bottom", 
 legend.justification = "bottom", plot.title = element_text(size = 20L, face = "bold", hjust = 0.5))
```
...
combined_plot<-(p1| p2) +
  plot_annotation(title="Combined plot example")
